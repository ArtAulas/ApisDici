...
#busca_linear(lista20,22)
#busca_linear(lista20,30)
...
def passo_da_busca_binaria(lista,procurando,menor_i,maior_i):
    ...
    if lista[meio] < procurando:
        return meio+1,maior_i
    if lista[meio] > procurando:
        return menor_i,meio-1

#listateste=[10,20,30,40,50,60,70,80,90]
#passo_da_busca_binaria(listateste,100,2,6)
...
def passo_melhor_da_busca_binaria(lista,procurando,menor_i,maior_i):
    meio=media_arredondada(menor_i,maior_i)
    interv=tam_do_intervalo(menor_i,maior_i)
    if interv==0:
        return False
    if interv==1:
        if lista[meio]!=procurando:
...
def busca_binaria(lista,procurado):
    comeco=0
    fim=len(lista)-1
    interv=tam_do_intervalo(comeco,fim)
