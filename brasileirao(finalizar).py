...
def nome_campeao(dados):
    id_champ=dados['fases']['2700']['classificacao']['grupo']['Único'][0]
    nome_champ=dados['equipes'][id_champ]['nome-comum']
    return nome_champ
...
def ids_dos_melhor_classificados(dados,numero_de_times):
    ids_classifica=[]
    list_classi=dados['fases']['2700']['classificacao']['grupo']['Único']
    for id in list_classi:
        ids_classifica.append(id)
        if len(ids_classifica)==numero_de_times:
            return ids_classifica
...
def classificados_libertadores(dados):
    ...
    for id in classificacoes:
        ...
        if len(ids_libertadores)==faixa:
            return ids_libertadores

#classificados_libertadores(dados2018)
'''
...
def nomes_classificados_libertadores(dados):
    ...
    classi=dados['fases']['2700']['classificacao']['grupo']['Único']

    ids_liber=[]
    for id in classi:
        ids_liber.append(id)
        if len(ids_liber)==faixa:
            break
    
    nomes_liber=[]
    for id in ids_liber:
        name=dados['equipes'][id]['nome-comum']
        nomes_liber.append(name)
    return nomes_liber

#nomes_classificados_libertadores(dados2018)
...
#ids_dos_times_de_um_jogo(dados2018,'102123')
